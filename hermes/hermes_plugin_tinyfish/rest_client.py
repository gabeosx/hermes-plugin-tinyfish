...
def close_browser_session(session_id: str, *, api_key: str, timeout: float = 15.0) -> bool:
    ...
    TinyFish returns a 2xx response, normally 204, for successful and
    idempotent termination. A 404 is not success: it can also mean the session
    belongs to a different API key. Retry only documented transient statuses
    and transport failures, keeping process-exit cleanup bounded.
    """
    ...
    try:
        ...
        if exc.response.status_code == 404:
            raise TinyFishWalletNotFound(
                "TinyFish has no wallet for this account (legacy billing or no Metronome customer yet)."
            ) from exc
...
def usage(*, api_key: str, timeout: float = 30.0) -> dict[str, Any]:
    """Compatibility wrapper for the wallet-backed usage command."""

    return wallet(api_key=api_key, timeout=timeout)
